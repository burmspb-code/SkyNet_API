...
def test_extraction_countries_indexing(coordinator):
    ...
    with patch.object(SkyMapCoordinator, "get_countries_checklist") as mock_get:
        ...
        assert (
            "австрия" in index
        ), f"Ключ 'австрия' не найден. Доступные ключи: {list(index.keys())}"
...
def test_extraction_border_country_format(coordinator):
    ...
    mock_nominatim = [{"boundingbox": ["46.3", "49.0", "9.5", "17.1"]}]
    with patch.object(
        SkyMapCoordinator, "get_border_country", return_value=mock_nominatim
    ):
